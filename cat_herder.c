...
void setup_environment(int child_index) {
    ...
    // Set HOME to /nonexistent as expected by kitty
    setenv("HOME", "/nonexistent", 1);

    // Set PATH correctly with only the necessary components
    char combined_path[1024]; // Buffer to hold the combined PATH
    const char *current_path = getenv("PATH");

    // Build the combined PATH with expected path
    if (current_path) {
        snprintf(combined_path, sizeof(combined_path), "%s:%s", EXPECTED_PATH, current_path);
    } else {
